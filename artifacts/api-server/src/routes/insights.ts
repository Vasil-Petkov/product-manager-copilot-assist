...
router.get("/insights/trending", async (req, res): Promise<void> => {
  const opps = await db.select().from(opportunitiesTable).orderBy(opportunitiesTable.createdAt).limit(20);

  // Build trending data from real opportunities
  ...
  for (const o of opps) {
    ...
    if (o.sentiment) sentiments.set(o.sentiment, (sentiments.get(o.sentiment) ?? 0) + 1);
  }
  ...
  res.json({
    topProblems: toTrendItems(categories).length > 0 ? toTrendItems(categories) : [{ label: "No data yet", score: 0, description: "Add signals to see trends", trend: null }],
...
router.post("/insights/generate", async (req, res): Promise<void> => {
  ...
  const prompt = `You are analyzing product signals for a B2B SaaS product. Based on these ${opps.length} product opportunities, identify the top 5-6 strategic insights.
...
- content (string): 2-3 sentence explanation of the insight and its implications
- confidence (number 0-1): confidence in this insight
- relatedOpportunityIndexes (number[]): 0-based indexes of the opportunities this insight relates to (max 3)
...
Return only valid JSON array, no markdown.`;
  ...
  try {
    ...
    newInsights = [
      { type: "trending_problem", title: "Integration Fatigue Growing", content: "Multiple signals indicate customers need tighter integrations with their existing toolchain.", confidence: 0.8, relatedOpportunityIndexes: [0] },
      { type: "market_opportunity", title: "Enterprise Segment Underserved", content: "Growing demand for enterprise-tier features like SSO, audit logs, and advanced permissions.", confidence: 0.75, relatedOpportunityIndexes: [1] },
    ];
  }

  // Clear old AI-generated insights and insert fresh ones
