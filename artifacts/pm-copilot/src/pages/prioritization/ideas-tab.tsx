  ...
  if (!items?.length) {
    return (
      <div className="text-center py-20 text-muted-foreground">
        No Product Ideas found. Add some in the Discovery module first.
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {items.map((item) => {
        ...
        const execData = (item as any).executiveData as Record<string, any> | null;
        // Get executiveData from the analysis stored in the list endpoint
        const vveQ = item.vveQuadrant as string | null;
        const quadrant = vveQ ? QUADRANT_LABELS[vveQ] : null;
            ...
            <CardContent className="p-5 flex flex-col flex-1 gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {item.riceScore?.score != null && (
                    <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded font-mono">
                      RICE {Number(item.riceScore.score).toFixed(1)}
                    </span>
                  )}
                  {item.iceScore?.score != null && (
                    <span className="text-xs bg-muted px-2 py-0.5 rounded font-mono">
                      ICE {Number(item.iceScore.score).toFixed(0)}
                    </span>
                  )}
                  {item.opportunityScore != null && (
                    <span className="text-xs bg-muted px-2 py-0.5 rounded font-mono">
                      OS {Number(item.opportunityScore).toFixed(1)}
                    </span>
                  )}
                  {quadrant && (
                    <Badge variant="outline" className={`text-[10px] ${quadrant.color}`}>{quadrant.label}</Badge>
                  )}
                </div>
              )}
              ...
              <div className="mt-auto pt-2">
                <Button
                  ...
                  onClick={() => handleAnalyze(opp.id, opp.title)}
                >
                  <Brain className="size-3.5" />
                  {item.analyzed ? "Re-analyze" : "Analyze with AI"}
                </Button>
              </div>
            </CardContent>
          </Card>
